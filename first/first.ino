...
{      
  ...
  Serial.println("Clear sleep bit in sensor");
...
int x, y, z;
int lastX, deltaX, lastY, deltaY, lastZ, deltaZ;
int flag;
#define GOINFACTOR 12000;
...
void loop()
{
  ...
  x = MPU9150_readSensor(MPU9150_ACCEL_XOUT_L,MPU9150_ACCEL_XOUT_H);
  y = MPU9150_readSensor(MPU9150_ACCEL_YOUT_L,MPU9150_ACCEL_YOUT_H);
  z = MPU9150_readSensor(MPU9150_ACCEL_ZOUT_L,MPU9150_ACCEL_ZOUT_H);
  deltaX = abs(x - lastX);
  deltaY = abs(y - lastY);
  deltaZ = abs(z - lastZ);
  ...
  Serial.write(isGoIn(deltaX, deltaY, deltaZ));
  */
  //Serial.print(".");
  
  if (isGoIn(deltaX, deltaY, deltaZ) ) {
    Serial.print("\nGot In\n");
    delay(4000);
  }
...
boolean isGoIn(int deltaX, int deltaY, int deltaZ) {
  
  int x = deltaX/GOINFACTOR;
  int y = deltaY/GOINFACTOR;
  int z = deltaZ/GOINFACTOR;
...
void MPU9150_setupCompass(){
  ...
  Serial.println("Wait for data at slave0");
